...
func Insert(c *gin.Context, userData authdtos.LoggedUserDTO, userRole enums.CourseUserRoleEnum) *common.ErrorResponse {
	...
	case enums.CourseItemTypeActivity:
		...
		err = tiptap.FindAndSaveRelations(transaction, userData.ID, reqData.ActivityDetail.Description, &innerCourseItem, "DescriptionFiles")
		if err != nil {
			return err
		}
		err = tiptap.FindAndSaveRelations(transaction, userData.ID, reqData.ActivityDetail.ExpectedResult, &innerCourseItem, "ExpectedResultFiles")
		if err != nil {
			return err
		}
